...
from pandas import DataFrame, options, read_excel, concat
...
def reset_ids(base_direc, idx, domTree, start_cell_id, time_intervel):
    ...
    data_0 = load_workbook(ospath.join(base_direc, "tracking", f"points_locations_{idx}.xlsx"))
    data_0.active
    the_sheet_0 = data_0.get_sheet_by_name("annotations")
    data_0.remove_sheet(the_sheet_0)
    data_0.create_sheet('annotations')
    data_0.save(ospath.join(base_direc, "tracking", f"points_locations_{idx}.xlsx"))
...
def reload_excel(directory, id, SpotsInFrame, ImageData, shape):
    ...
    if added_IDs != []:
        data_0 = load_workbook(ospath.join(directory, "tracking", f"points_locations_{id}.xlsx"))
        data_0.active
        the_sheet_0 = data_0.get_sheet_by_name("annotations")
        ...
        for add_id in added_IDs:
            ...
            max_row_0 += 1
        data_0.save(ospath.join(directory, "tracking", f"points_locations_{id}.xlsx"))
    if deleted_IDs != []:
        data_0 = load_workbook(ospath.join(directory, "tracking", f"points_locations_{id}.xlsx"))
        data_0.active
        the_sheet_0 = data_0.get_sheet_by_name("annotations")
...
def refine_excel(directory, idx, index, color):
    data_0 = load_workbook(ospath.join(directory, "tracking", f"points_locations_{idx}.xlsx"))
    data_0.active
    the_sheet_0 = data_0.get_sheet_by_name("annotations")
    for i in index:
        ix = i.index()
        the_sheet_0.cell(row=ix+2, column=6).value = color
    data_0.save(ospath.join(directory, "tracking", f"points_locations_{idx}.xlsx"))
...
def xz_lines_from_excel(directory, volume):
    data = load_workbook(ospath.join(directory, "tracking", f"points_locations_{volume}.xlsx"))
    data.active
    the_sheet = data.get_sheet_by_name("annotations")
...
def line_correspondence(directory, volume, rank):
    ...
    data.save(ospath.join(directory, "tracking", f"points_locations_{volume}.xlsx"))
...
def get_mitotic_color(directory):
    path = ospath.join(directory, "tracking")
    df = None
...
def link_spots_by_y(cells_0, cells_1, id, directory, nethermost_0, nethermost_index_0, off_set=[0 for _ in range(8)]):
    data_0 = DataFrame(read_excel(ospath.join(directory, "tracking", f"points_locations_{id}.xlsx"), engine='openpyxl', ))
    nethermost_1 = []
    nethermost_index_1 = []
    zs = []
    xs = []
    ...
    for i in range(8):
        ...
        for j in range(nethermost_index_0[i], end):
            ...
            if start >= length_1:
                break
    data_0.to_excel(ospath.join(directory, "tracking", f"points_locations_{id}.xlsx"), sheet_name='annotations',
                    index=False)
...
def create_blank_xml(directory, img_shape):
    ...
    if not ospath.exists(ospath.join(directory, f"filename[0].xml")):
